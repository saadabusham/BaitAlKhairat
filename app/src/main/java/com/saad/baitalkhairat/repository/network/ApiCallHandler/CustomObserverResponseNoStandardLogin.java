...
public class CustomObserverResponseNoStandardLogin<T> extends CustomDialogUtils implements Observer<Response<T>> {

    APICallBack apiCallBack;
    Context context;
    boolean withProgress;

    public CustomObserverResponseNoStandardLogin(Context mContext, boolean withProgress, APICallBack apiCallBack) {
    ...
    public void onNext(Response<T> generalResponseResponse) {
        ...
        } else if (generalResponseResponse.code() == 422 &&
                generalResponseResponse.errorBody() != null) {
            try {
                JSONObject jsonObject = new JSONObject(generalResponseResponse.errorBody().string());
                this.apiCallBack.onError(jsonObject.has("errors") ?
                        jsonObject.getString("errors") : "", jsonObject.has("errors") ? 7777 : 0);
            } catch (IOException e) {
                e.printStackTrace();
